...
# the fixed test credentials (user app / pass test / db app_test — the
...
# Container name is derived from the repo dir so two apps on one machine
# don't fight; the HOST port (default 5433) avoids any real local Postgres
# on 5432. Override with TEST_DB_PORT if 5433 is taken.
PG_CONTAINER="$(basename "$PWD")-test-pg"
...
if ! docker ps --format '{{.Names}}' | grep -qx "$PG_CONTAINER"; then
  if docker ps -a --format '{{.Names}}' | grep -qx "$PG_CONTAINER"; then
    ...
    docker run -d --name "$PG_CONTAINER" -p "${PG_PORT}:5432" \
      -e POSTGRES_USER=app -e POSTGRES_PASSWORD=test -e POSTGRES_DB=app_test \
      postgres:18-alpine >/dev/null
  fi
fi
until docker exec "$PG_CONTAINER" pg_isready -q -U app -d app_test 2>/dev/null; do
  sleep 0.5
done
